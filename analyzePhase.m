function analyzePhase(datedir, experimentNumber,filemodifier)
if nargin<3
    filemodifier=[];
end
...
for trial=1:numberTrials
    ...
    meanData=mean(tcData,3);
    deltaF=bsxfun(@rdivide,bsxfun(@minus,tcData,meanData),meanData);
    deltaF=repmat(bsxfun(@minus,deltaF,mean(mean(deltaF,2),1)),[1,1,3]);
    %     deltaF=repmat(bsxfun(@minus,tcData,mean(tcData,3)),[1,1,3]);
    ...
    save(fullfile(filebase,strcat('Condition_',int2str(c)),...
        strcat('phaseResults_',filemodifier,datedir,'_',int2str(experimentNumber),'_c_',int2str(c),'_trial_',int2str(trial))),...
    ...
    saveas(hAnalysis,fullfile(filebase,strcat('Condition_',int2str(c)),...
        strcat('phaseFig_',filemodifier,datedir,'_',int2str(experimentNumber),'_c_',int2str(c),'_trial_',int2str(trial),'.fig')));
    saveas(hAnalysis,fullfile(filebase,strcat('Condition_',int2str(c)),...
        strcat('phaseFig_',filemodifier,datedir,'_',int2str(experimentNumber),'_c_',int2str(c),'_trial_',int2str(trial),'.jpg')));    
    ...
    movie2avi(mov,fullfile(filebase,strcat('Condition_',int2str(c)),...
        strcat('phaseMov',datedir,'_',int2str(experimentNumber),'_c_',int2str(c),'_trial_',int2str(trial),'.avi')),'fps',5);
    end
end